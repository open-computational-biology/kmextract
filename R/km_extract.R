...
.km_find_ticks <- function(g, axes, dark_thresh) {
  dark <- g < dark_thresh
  h <- nrow(g); w <- ncol(g)
  xb <- dark[pmin(h, (axes$x_row + 3L):(axes$x_row + 10L)), , drop = FALSE]
  x_px <- .km_group_centers(setdiff(which(colSums(xb) > 0), 1:axes$y_col))
  yb <- dark[, pmax(1L, (axes$y_col - 10L)):(axes$y_col - 3L), drop = FALSE]
  y_px <- .km_group_centers(intersect(which(rowSums(yb) > 0), 1:(axes$x_row - 1L)))
